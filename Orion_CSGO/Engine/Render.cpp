...
namespace Engine
{
	...
	void CRender::DrawLine( int x1 , int y1 , int x2 , int y2 , SDK::Color color )
	{
	...
	void CRender::DrawBox( int x , int y , int w , int h , SDK::Color color )
	{
	...
	void CRender::DrawFillBox( int x , int y , int w , int h , SDK::Color color )
	{
	...
	void CRender::DrawOutlineBox( int x , int y , int w , int h , SDK::Color color )
	{
		DrawBox( x , y , w , h , SDK::Color::Black() );
		DrawBox( x + 1 , y + 1 , w - 2 , h - 2 , color );
		DrawBox( x + 2 , y + 2 , w - 4 , h - 4 , SDK::Color::Black() );
	}

	void CRender::DrawCoalBox( int x , int y , int w , int h , SDK::Color color )
	{
		int iw = w / 4;
		int ih = h / 4;
		// top
	...
	void CRender::DrawOutlineCoalBox( int x , int y , int w , int h , SDK::Color color )
	{
		int iw = w / 4;
		int ih = h / 4;

		DrawCoalBox( x , y , w , h , SDK::Color::Black() );
		
		// top left
		DrawLine( x + 1 , y + 1 , x + iw , y + 1 , color );
		DrawLine( x + iw , y , x + iw , y + 2 , SDK::Color::Black() );
		DrawLine( x + 1 , y + 1 , x + 1 , y + ih , color );
		DrawLine( x , y + ih , x + 2 , y + ih , SDK::Color::Black() );

		// top right
		DrawLine( x + w - iw , y + 1 , x + w - 1 , y + 1 , color );
		DrawLine( x + w - iw - 1 , y , x + w - iw , y + 2 , SDK::Color::Black() );
		DrawLine( x + w - 2 , y + 1 , x + w - 2 , y + ih , color );
		DrawLine( x + w - 2 , y + ih , x + w , y + ih , SDK::Color::Black() );

		// bottom left
		DrawLine( x + 1 , y + h - ih , x + 1 , y + h , color );
		DrawLine( x , y + h - ih - 1 , x + 2 , y + h - ih - 1 , SDK::Color::Black() );
		DrawLine( x + 1 , y + h - 1 , x + iw , y + h - 1 , color );
		DrawLine( x + iw , y + h - 1 , x + iw , y + h + 1 , SDK::Color::Black() );

		// bottom right
		DrawLine( x + w - iw , y + h - 1 , x + w - 1 , y + h - 1 , color );
		DrawLine( x + w - iw - 1 , y + h - 1 , x + w - iw , y + h + 1 , SDK::Color::Black() );
		DrawLine( x + w - 2 , y + h - ih , x + w - 2 , y + h , color );
		DrawLine( x + w - 2 , y + h - ih - 1 , x + w , y + h - ih - 1 , SDK::Color::Black() );

		DrawCoalBox( x + 2 , y + 2 , w - 4 , h - 4 , SDK::Color::Black() );
	}

	void CRender::DrawWindow( int x , int y , int w , int h , SDK::Color Border , SDK::Color Background )
	{
		DrawBox( x , y , w , h , Border );
		DrawFillBox( x + 1 , y + 1 , w - 2 , h - 2 , Background );
	}

	void CRender::DrawHorBar( int x , int y , int w , int h , int val , SDK::Color color1 , SDK::Color color2 )
	{
		if ( val > 100 ) val = 100;
		int size = ( w * val ) / 100;
	...
	void CRender::DrawVerBar( int x , int y , int w , int h , int val , SDK::Color color1 , SDK::Color color2 )
	{
		if ( val > 100 ) val = 100;
		int size = ( h * val ) / 100;
		
		DrawBox( x , y , w , h , SDK::Color( 0 , 0 , 0 ) );
		DrawFillBox( x + 1 , y + 1 , w - 2 , h - 2 , color2 );
	...
	void CRender::Text( int x , int y , bool center , bool shadow , SDK::Color color , const char* format , ... )
	{
		if ( m_pStateBlockText )
